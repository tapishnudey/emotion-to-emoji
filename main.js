...
function takesnapshot(){
    ...
    document.getElementById("result").innerHTML='<img id="snap_result" src="'+data_uri+'"/>';
    });
}

classifier=ml5.imageClassifier('https://teachablemachine.withgoogle.com/models/5EopLXPqu/model.json', modelLoded);
...
function check(){
    img=document.getElementById('snap_result');
    classifier.classify(img ,got_result);
}

function got_result(eror, results){
    if (eror){
        console.error(error);
    }else{
        console.log(results);
